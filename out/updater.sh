#!/tmp/busybox sh
#
# Kernel Installer Script for Vibrant
# (c) 2013 by Team Passion
#
...
if /tmp/busybox test -e /dev/block/bml7 ; then
    ...
    if $IS_GSM ; then
        ...
        # if modem.bin doesn't exist on radio partition, warn the user
        if ! /tmp/busybox test -e /radio/modem.bin ; then
            /tmp/busybox echo "Cannot Find Modem. Please flash separately!!"
        fi

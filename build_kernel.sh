...
if [ -e $KERNELDIR/arch/arm/boot/zImage ]; then
	...
	rm $KERNELDIR/out/Mackay-Kernel_* >> /dev/null;
	GETVER=`grep 'Mackay-Kernel_v.*' arch/arm/configs/${KERNEL_CONFIG} | sed 's/.*_.//g' | sed 's/".*//g'`
	cp $KERNELDIR/boot.img /$KERNELDIR/out/
	cd $KERNELDIR/out/
	zip -r Mackay-Kernel_v${GETVER}-`date +"[%m-%d]-[%H-%M]"`.zip .
	...
	if [ "$push_ok" == "y" ] || [ "$push_ok" == "Y" ]; then
		...
		adb push $KERNELDIR/out/Mackay-Kernel_v*.zip /sdcard/
